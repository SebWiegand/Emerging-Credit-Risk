...
# --- Add Cong et al. code folder to sys.path so imports work when running from Scripts/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
TEXT_ANALYTICS_DIR = os.path.dirname(SCRIPT_DIR)  # .../Text analytics
CONG_REP_DIR = os.path.join(TEXT_ANALYTICS_DIR, "Cong et al. rep")

if CONG_REP_DIR not in sys.path:
    sys.path.insert(0, CONG_REP_DIR)
...
# Project root = the repo root (folder that contains "Reports" and "Text analytics")
# This file lives in: <repo>/Text analytics/Scripts/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
TEXT_ANALYTICS_DIR = os.path.dirname(SCRIPT_DIR)          # <repo>/Text analytics
PROJECT_ROOT = os.path.dirname(TEXT_ANALYTICS_DIR)        # <repo>

# Folder with your annual reports (your PDFs are stored under Text analytics/Reports)
...
def main():
    ...
    if len(report_paragraphs) == 0:
        raise RuntimeError(
            f"No text paragraphs were extracted. Check that PDFs exist in: {reports_folder} "
            f"and that your filenames match page_ranges keys."
        )

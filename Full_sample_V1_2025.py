...
OUTPUT_TAG = "Run5_2025_final_186"
...
TOPIC_SINGLE = "topic_loading_186"
TOPIC_PLOT_TITLE_TEMPLATE = "Gas"
...
STATIC_PLOT_TITLE = "Static risk model without Covid (2006-2008 baseline, 4-quarter smoothed average topic z-score)"
...
if all(c in panel.columns for c in ctrl_cols):
    ...
    # --------------------------------------------------
    # Pearson correlation matrix for controls (panel level)
    # --------------------------------------------------
    try:
        ctrl_corr_df = panel[ctrl_cols].copy()

        # Ensure numeric
        ctrl_corr_df = ctrl_corr_df.apply(pd.to_numeric, errors="coerce")

        # Compute Pearson correlation
        ctrl_corr = ctrl_corr_df.corr(method="pearson")

        out_dir = HERE / "Output"
        out_dir.mkdir(exist_ok=True)

        corr_csv = out_dir / f"controls_correlation_matrix_{OUTPUT_TAG}.csv"
        ctrl_corr.to_csv(corr_csv)

        print("Saved Pearson correlation matrix for controls:", corr_csv)

        # Print a compact preview in console
        with pd.option_context("display.max_rows", 30, "display.max_columns", 30, "display.width", 200):
            print("\nPearson correlation matrix (controls):")
            print(ctrl_corr.round(3))

    except Exception as e:
        print("WARNING: Failed to compute control correlation matrix:", e)
...
#
# NOTE: Use HC1 robust standard errors consistently; HC3 can become unstable in some high-leverage quarterly cross-sections.
# ============================
...
for q, g in panel.groupby("quarter", sort=True):
    ...
    base = sm.OLS(y0, X_base).fit(cov_type="HC1")
    ...
    full_all = sm.OLS(y0, X_full_all).fit(cov_type="HC1")
    ...
    for idx_k, k in enumerate(topic_cols):
        ...
        if S0.shape[1] == 1:
            ...
            # Drop-one model with heteroskedasticity-robust standard errors (HC1)
            m_minus = sm.OLS(y0, X_minus).fit(cov_type="HC1")
...
baseline = results[
    results["year_q"].between(BASELINE_START_YEAR, BASELINE_END_YEAR)
].copy()

...
# Baseline window for z-scores (pre-COVID calibration)
...
# If True, run a controls-only model (no topic regressors, no topic z-scores)
RUN_CONTROLS_ONLY = True
...
MIN_BASELINE_SD = 0  # small floor to avoid NaN z-scores when baseline dR2 is near-constant
...
if TF_PATH is None:
    raise FileNotFoundError(
        "Could not find extraction summary CSV. Tried:\n"
...
if RUN_CONTROLS_ONLY:
    topic_cols = []
    print("Selected 0 topics for regression (controls-only mode).")
    ...
    print(f"Selected {len(topic_cols)} topics for regression.")
...
# ============================
if RUN_CONTROLS_ONLY:
    tf_firm_year = tf[["bank", "year"]].drop_duplicates().copy()
...
# ============================
if RUN_VIF_TEST and not RUN_CONTROLS_ONLY:
...
if results.empty:
    raise RuntimeError("Controls-only regression results are empty. Check control coverage.")
# Diagnostics: show which years actually have regression output
results["year_q"] = results["quarter"].astype(str).str[:4].astype(int)
year_counts = results.groupby("year_q")["R2_base"].size().sort_index()
...
out_csv = out_dir / f"controls_only_quarterly_r2_{OUTPUT_TAG}.csv"
results.to_csv(out_csv, index=False)
print("Saved:", out_csv)

# Plot: Controls-only quarterly R2
controls_r2 = results.copy()
controls_r2["quarter_ts"] = pd.PeriodIndex(controls_r2["quarter"].astype(str), freq="Q").to_timestamp()
controls_r2 = controls_r2.sort_values("quarter_ts")
controls_r2["R2_smooth4"] = controls_r2["R2_base"].rolling(window=4, min_periods=4).mean()

try:
    controls_r2_plot = controls_r2.copy()
    controls_r2_plot["year_plot"] = controls_r2_plot["quarter_ts"].dt.year
    controls_r2_plot = controls_r2_plot[
        (controls_r2_plot["year_plot"] >= PLOT_START_YEAR) &
        (controls_r2_plot["year_plot"] <= PLOT_END_YEAR)
    ].copy()

    plt.figure(figsize=(14, 4))
    x = np.arange(len(controls_r2_plot))
    plt.bar(x, controls_r2_plot["R2_smooth4"].values)

    plt.title("Controls-only model (4-quarter smoothed quarterly $R^2$)")
    plt.xlabel("Quarter")
    plt.ylabel("$R^2$")

    years = controls_r2_plot["quarter_ts"].dt.year
    year_idx = years.ne(years.shift()).to_numpy().nonzero()[0]
    plt.xticks(year_idx, years.iloc[year_idx].astype(str).tolist(), rotation=0)

    plt.tight_layout()
    out_png = out_dir / f"controls_only_r2_{OUTPUT_TAG}.png"
    ...
    print("WARNING: Failed to plot controls-only model:", e)
...
if RUN_DIAGNOSTICS:
    ...
    if (not RUN_CONTROLS_ONLY) and "dR2" in results.columns:
    ...
    if (not RUN_CONTROLS_ONLY) and set(["topic","dR2"]).issubset(results.columns):
        topic_rank = (
    ...
    if (not RUN_CONTROLS_ONLY) and "TOPIC_SINGLE" in globals() and "topic" in results.columns and TOPIC_SINGLE in results["topic"].unique():
    ...
    if (not RUN_CONTROLS_ONLY) and "year_q" in results.columns:

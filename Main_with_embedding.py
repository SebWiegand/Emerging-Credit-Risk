...
def train_openai_embeddings(df):
    ...
    Input:
    - df : DataFrame with a 'tokens' column (list of words per document)

    Output:
    - vocab            : sorted unique vocabulary list
    - embedding_matrix : numpy array of shape (V, D)
    """

    # ------------------------------------------------------------
    # 1. Build vocabulary (unique words across all documents)
    # ------------------------------------------------------------
    tokenized_docs = df["tokens"].tolist()
    print("Number of documents going into embedding:", len(tokenized_docs))

    # Flatten token lists to get vocabulary
    vocab = sorted(set(word for doc in tokenized_docs for word in doc))
    print(f"Vocabulary size: {len(vocab)} words")

    if len(vocab) == 0:
        raise RuntimeError("No tokens found — cannot embed.")

    # ------------------------------------------------------------
    # 2. Embed vocabulary in batches (OpenAI API limit)
    # ------------------------------------------------------------
    BATCH_SIZE = 512            # safe for OpenAI embeddings
    model_name = "text-embedding-3-small"

    embedding_dim = None
    all_vectors = []

    print("Embedding words using OpenAI…")

    for start in range(0, len(vocab), BATCH_SIZE):
        end = start + BATCH_SIZE
        batch = vocab[start:end]
        ...
        print(f" Embedded {end}/{len(vocab)} words...")
...
def main():
    ...
    # ---------------------------------------------------------
    # STEP 4: Create OpenAI embeddings on PARAGRAPHS
    # ---------------------------------------------------------
    print("\n=== STEP 4: Create OpenAI embeddings on paragraph-level tokens ===")

...
# ============================================================
# ------------------------------------------------------------
# Separate text-window year from reported output year.
# TEXT_TARGET_YEAR controls which annual-report extraction summary is used.
# OUTPUT_TARGET_YEAR controls which covariance year is reported in rankings/tables.
# Example here: use 2020-2024 text to explain/report 2025 quarters.
# ------------------------------------------------------------
...
MIN_BASELINE_SD = 0.00000  # small floor to avoid NaN z-scores when baseline dR2 is near-constant
...
print(
    ...
    f"(TEXT_TARGET_YEAR = {TEXT_TARGET_YEAR}, OUTPUT_TARGET_YEAR = {OUTPUT_TARGET_YEAR})"
)
...
# --------------------------------------------------
# Target-year ranking: report the highest z-score topics in OUTPUT_TARGET_YEAR
# --------------------------------------------------
target_year_results = results[results["year_q"] == OUTPUT_TARGET_YEAR].copy()

if target_year_results.empty:
    warnings.warn(
        f"No regression rows found for output year {OUTPUT_TARGET_YEAR}. "
...
print(f"Top {TOP_Z_COUNT} z-score topics PER QUARTER in output year {OUTPUT_TARGET_YEAR}:")
...
print(f"Top {TOP_Z_COUNT} UNIQUE topics in output year {OUTPUT_TARGET_YEAR} (aggregated across quarters):")
...
top_z_csv = out_dir / f"top_z_topics_output_{OUTPUT_TARGET_YEAR}_text_{DYNAMIC_TF_RUN_LABEL}_{OUTPUT_TAG}.csv"
top_target_year.to_csv(top_z_csv, index=False)
print("Saved target-year top z rows per quarter:", top_z_csv)

top_z_topic_csv = out_dir / f"top_z_topics_unique_output_{OUTPUT_TARGET_YEAR}_text_{DYNAMIC_TF_RUN_LABEL}_{OUTPUT_TAG}.csv"
